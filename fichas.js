const fichas = [
    ...
    { nombre: "妮老师", pinyin: "Nī Lǎoshī", seguidores: 3.46, likes: 110, ratio: 31.84, origen: "Fujian", provincia: "福建省", coord: [119.30, 26.08], archivo: "bailarina.jpg", nicho: "Humor/Musical" },
    ...
    { nombre: "猫姨", pinyin: "Māo Yí", seguidores: 2.92, likes: 74.2, ratio: 25.44, origen: "Sichuan", provincia: "四川省", coord: [104.07, 30.57], archivo: "Miaomiao.jpg", nicho: "Baile urbano" },
    { nombre: "Angier安琦", pinyin: "Angier Ān Qí", seguidores: 2.80, likes: 60, ratio: 21.43, origen: "Liaoning → Kunming", provincia: "辽宁省", coord: [123.43, 41.80], archivo: "ngjkkm.jpg", nicho: "Baile urbano" },
    ...
    { nombre: "墨尔本获宝", pinyin: "Mò'ěrběn Huò Bǎo", seguidores: 1.92, likes: 5.68, ratio: 2.96, origen: "Melbourne, Australia", provincia: "", coord: null, archivo: "cocinera.jpg", nicho: "Lifestyle" },
    { nombre: "小七。", pinyin: "Xiǎo Qī", seguidores: 1.00, likes: 5.68, ratio: 5.68, origen: "Sichuan", provincia: "四川省", coord: [104.07, 30.57], archivo: "xiao qi.jpg", nicho: "Lifestyle" },
    { nombre: "刘书含", pinyin: "Liú Shūhán", seguidores: 0.992, likes: 16.47, ratio: 16.61, origen: "Liaoning", provincia: "辽宁省", coord: [123.43, 41.80], archivo: "Rose.jpg", nicho: "Ópera moderna" },
    { nombre: "秋月凉L", pinyin: "Qiū Yuè Liáng", seguidores: 0.94, likes: 15.26, ratio: 16.24, origen: "Sichuan", provincia: "四川省", coord: [104.07, 30.57], archivo: "liang.jpg", nicho: "Hanfu cinematográfico" },
    { nombre: "刘荇羽", pinyin: "Liú Xíngyǔ", seguidores: 0.562, likes: 7.2, ratio: 12.80, origen: "Malasia (Tai Shan)", provincia: "", coord: null, archivo: "muralista.jpg", nicho: "Murales" },
    ...
    { nombre: "小鹤owo", pinyin: "Xiǎo Hè owo", seguidores: 1.531, likes: 20.08, ratio: 13.12, origen: "Xiamen, Fujian", provincia: "福建省", coord: [119.30, 26.08], archivo: "owo (2).jpg", nicho: "Cosplay + gaming" }
];

function actualizarPagina() {
    const lista = [...fichas].sort((a,b) => b.seguidores - a.seguidores);
    document.getElementById('total-fichas') && (document.getElementById('total-fichas').textContent = fichas.length);
    document.getElementById('max-seguidores') && (document.getElementById('max-seguidores').textContent = lista[0].seguidores.toFixed(2) + 'M');
    const regiones = new Set(fichas.map(f => f.origen.split(',')[0].trim()));
    document.getElementById('total-regiones') && (document.getElementById('total-regiones').textContent = regiones.size);
    document.getElementById('footer-fichas') && (document.getElementById('footer-fichas').textContent = fichas.length + ' creadoras · 30,500+ comunidad');
    
    const tbody = document.getElementById('tabla-body');
    if (tbody) {
        tbody.innerHTML = '';
        lista.forEach(f => tbody.innerHTML += `<tr><td>${f.nombre} (${f.pinyin})</td><td>${f.seguidores.toFixed(3)}M</td><td>${f.likes}M</td><td>${f.ratio.toFixed(2)}</td><td>${f.origen}</td><td>${f.nicho}</td></tr>`);
    }
